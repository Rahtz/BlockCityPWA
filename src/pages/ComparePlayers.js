...
const ComparePlayers = () => {
  ...
  async function fetchPlayerStats(playerId, season) {
    ...
    const { data, error } = await supabase
      .from("stats")
      .select(
        "Season, Points, Rebounds, Assists, Steals, Blocks, FeildGoalsAttempted, FeildGoalsMade, ThreePointersAttempted, ThreePointersMade, FreeThrowsAttempted, FreeThrowsMade, Turnovers, MvpPoints"
      )
  ...
  async function getStats() {
    const player1SeasonStats = await fetchPlayerStats(
      player1Id,
      selectedSeason
    );
    const player2SeasonStats = await fetchPlayerStats(
      player2Id,
      selectedSeason
    );
  ...
  async function getDistinctSeasons() {
    const { data, error } = await supabase.from("stats").select("Season");
    ...
    const seasons = [
      ...new Set(data.map((item) => String(item.Season).substring(0, 4))),
    ];
  ...
  function calculateAverage(data, property) {
    ...
    // If "All Seasons" is selected, calculate average for all seasons
    if (!selectedSeason) {
      const allSeasonsData = data.flat();
      const sum = allSeasonsData.reduce(
        (total, item) => total + item[property],
        0
      );
      return allSeasonsData.length > 0 ? sum / allSeasonsData.length : 0;
    }

    // If a specific season is selected, calculate average for that season
    const filteredData = data
      .flat()
      .filter((item) => item.Season === parseInt(selectedSeason));
        ...
        <tbody>
            ...
            <td>
              {(
                (calculateAverage(stats[0], "FeildGoalsMade").toFixed(1) /
                  calculateAverage(stats[0], "FeildGoalsAttempted").toFixed(
                    1
                  )) *
                100
              ).toFixed(1) + "%"}
            ...
            <td>
              {(
                (calculateAverage(stats[1], "FeildGoalsMade").toFixed(1) /
                  calculateAverage(stats[1], "FeildGoalsAttempted").toFixed(
                    1
                  )) *
                100
              ).toFixed(1) + "%"}
          ...
          <tr>
            ...
            <td>
              {(
                (calculateAverage(stats[0], "ThreePointersMade").toFixed(1) /
                  calculateAverage(stats[0], "ThreePointersAttempted").toFixed(
                    1
                  )) *
                100
              ).toFixed(1) + "%"}
            ...
            <td>
              {(
                (calculateAverage(stats[1], "ThreePointersMade").toFixed(1) /
                  calculateAverage(stats[1], "ThreePointersAttempted").toFixed(
                    1
                  )) *
                100
              ).toFixed(1) + "%"}
            ...
            <td>
              {(
                (calculateAverage(stats[0], "FreeThrowsMade").toFixed(1) /
                  calculateAverage(stats[0], "FreeThrowsAttempted").toFixed(
                    1
                  )) *
                100
              ).toFixed(1) + "%"}
            ...
            <td>
              {(
                (calculateAverage(stats[1], "FreeThrowsMade").toFixed(1) /
                  calculateAverage(stats[1], "FreeThrowsAttempted").toFixed(
                    1
                  )) *
                100
              ).toFixed(1) + "%"}
